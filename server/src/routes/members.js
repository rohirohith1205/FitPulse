...
router.post("/", async (req, res, next) => {
  try {
    ...
    if (parsed.data.trainerId) {
      requireValidObjectId(parsed.data.trainerId, "trainerId");
      const memberCount = await Member.countDocuments({ trainerId: parsed.data.trainerId });
      if (memberCount >= 3) {
        const err = new Error("Trainer already has maximum 3 members assigned.");
        err.status = 400;
        err.code = "VALIDATION_ERROR";
        throw err;
      }
    ...
    const created = await Member.create({
      ...
      trainerId: parsed.data.trainerId || undefined,
...
router.patch("/:id", async (req, res, next) => {
  try {
    ...
    if (update.trainerId) {
      requireValidObjectId(update.trainerId, "trainerId");
      
      // If trainer is changing
      const oldMember = await Member.findById(req.params.id);
      if (oldMember.trainerId?.toString() !== update.trainerId) {
         const memberCount = await Member.countDocuments({ trainerId: update.trainerId });
         if (memberCount >= 3) {
            const err = new Error("Trainer already has maximum 3 members assigned.");
            err.status = 400;
            err.code = "VALIDATION_ERROR";
    ...
    } else if (update.trainerId === "") {
        update.trainerId = null; // Unassign trainer
    }
